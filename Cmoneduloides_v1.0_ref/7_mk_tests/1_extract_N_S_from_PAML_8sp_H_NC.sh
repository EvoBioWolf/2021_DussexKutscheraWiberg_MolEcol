...
results="/proj/b2013182/nobackup/bams/Cmon_Ref/PAML/Ns/genes_80_8sp_H_NC/results" # insert path to directory with results files
mktdir="/proj/b2013182/nobackup/POPseq/mk_tests" # insert path for directory for McDonald-Kreitman tests

cd $mktdir
touch mkt_table_N_S_80_8sp_H_NC.txt
echo 'gene N*dN S*dS' >> mkt_table_N_S_80_8sp_H_NC.txt
cd $results
...
if [[ $tab != "" ]]
then
echo $f $tab >> $mktdir/mkt_table_N_S_80_8sp_H_NC.txt
fi
done
